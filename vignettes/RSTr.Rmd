...
If you want to learn more about `mstcar()` and the other initialization functions, read `vignette("RSTr-initialization")`.

### `run_sampler()` {#sec-samples}

THIS SECTION IS DEPRECATED. running the *car() functions automatically runs the sampler now. Will edit this later.
...
After `run_sampler()` is done running (i.e., generates 6000 iterations) and your samples have been saved to `mod_test`, you can bring the samples into R using the `load_samples()` function. We can pull in samples from any of our available parameters, but let's pull in the outputs for `lambda`, our rate estimates:
...
This creates an `array` object with median estimates for `lambda` along each region, group, and year:
