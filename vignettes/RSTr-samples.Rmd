...
After initializing the model with `*car()`, you can gather and load samples using the `run_sampler()` and `load_samples()` functions. Finally, using some basic plotting, we can look at our results and make sure they make sense.
...
Now, we have a standalone array for our age-standardized 35-64 age group. Similarly to `aggregate_groups()`, we can alternatively consolidate this into our main `samples` array by adding in values for the `bind_new` and `new_name` arguments:
