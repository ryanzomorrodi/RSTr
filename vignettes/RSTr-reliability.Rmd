...
In the previous vignette, we introduced the `get_estimates()` function and discussed how to age-standardize with `age_standardize()`. In this vignette, we will take a deep dive into estimate reliability and showcase how we can use reliability criteria to suppress our estimates with `suppress_estimates()`.

## Estimate reliability

Though `RSTr` is designed to stabilize low-population regions, there is a limit to the amount of information the model can gather, and estimates from exceedingly low-population regions may be over-smoothed. To address these issues, we can establish criteria that indicate whether our estimates are reliable enough to display. Reliability can be easily tested in CAR models using two criteria:
...
Before showcasing how to get these reliability metrics, let's quickly compare the traceplots of high- and low-population counties to see the stability of the samples:

```{r}
knitr::include_graphics("vignettes/reliability-figures.png")
```
